...
/*
* The present code utilizes KMP to find all disjoint exact matches between given vectors.
* Also it uses dynamic programming to find the longest approximate match.
* Based on these two results we judge if plagiarism is present or not.
*/
...
private:
    ...
    static int findNextUnmatchedPart(int patsize, int txtsize, int start, const std::unordered_set<int> &matchedIndices)
    {
        bool found = 0;
        int i = start;
...
public:
    // MAIN Function
    static int KMPSearch(const std::vector<int> &pat, const std::vector<int> &txt, std::unordered_set<int> &matchedIndices)
    {
        int j = 0;
        int m = pat.size();
        int n = txt.size();
        ...
        while (i < n)
        {
            // If the current index is already matched, we need to find next unmatched part
            // and then restart kmp by setting j=0
            ...
            if (pat[j] == txt[i])
            {
                ...
                if (j == m) 
                {
                    ...
                    for (int k = i - j; k < i; k++) matchedIndices.insert(k);
    ...
    std::vector<int> longest_approximate_match(const std::vector<int> &vec1, const std::vector<int> &vec2)
    {
        ...
        for(int k=0; k<indSize; k++){
            for(int h=indSize-1; h>k; h--){
                ...
                if(minStringLen<30) break;
                
                int maxStringLen=std::max((ind1[h]-ind1[k]),(ind2[h]-ind2[k]));
                double fracMatch=double(h-k+1)/(maxStringLen+1);

                // Even if match found is greater than 80%, we should find the longest approximate match,
                // by extending it as much as possible such that accuracy remains greater than 80%
                int ans=std::min(std::min((int)((maxStringLen+1)*(fracMatch/0.8)),m),n); 

                // Fraction of matching subsequence should be at least 80% of longer substring    
                if(fracMatch>0.8 && ans>ans1)
                {
                    ans1=ans;
                    ans2=ind1[k];
                    ans3=ind2[k];
...
int total_match(const std::vector<int> &submission1, const std::vector<int> &submission2){
    ...
    for (int i = 0; i < (int)submission1.size(); ++i)
    {
        ...
        {
            if (i + len > submission1.size()) continue; // Remove out of bound errors
                                     ...
                                     std::vector<int> &submission2)
{
    ...
    double fracApproxPlag = approxMatch[0]/(double(std::max(submission1.size(), submission2.size())));
    int hasPlag = (fracExactPlag > 0.2 || fracApproxPlag > 0.5) ;
